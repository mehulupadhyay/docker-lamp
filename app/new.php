...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_POST["admin"])) { $admin = $_POST["admin"] ? 1 : 0; } else { $admin = 0; }
    if(isset($_POST["manager"])) { $manager = $_POST["manager"] ? 1 : 0; } else { $manager = 0; }

    // Check input errors before inserting in database
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($project_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO users (username, password, project, admin, manager) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sssss", $param_username, $param_password, $param_project, $param_admin, $param_manager);
